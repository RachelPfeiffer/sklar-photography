...
function wds_slider_ready(){jQuery(".wds_slider_cont").each(function(){var wds=jQuery(this).attr("data-wds");if(wds_params[wds].current_image_url!=''){jQuery('<img />').attr("src",wds_params[wds].current_image_url).on('load',function(){jQuery(this).remove();wds_ready_func(wds)})}else{wds_ready_func(wds)}
...
if(!(wds_params[wds].carousel!=0||wds_data[key].is_video!='image')){jQuery('<img />').attr("src",wds_data[key].image_url).on('load',function(){jQuery(this).remove();wds_change_image_when_loaded(wds,current_key,key,wds_data,from_effect,btn)}).on('error',function(){jQuery(this).remove();wds_change_image_when_loaded(wds,current_key,key,wds_data,from_effect,btn)})}else{wds_change_image_when_loaded(wds,current_key,key,wds_data,from_effect,btn)}}
...
if(btn==""){var direction="right";var int_curr_key=parseInt(wds_params[wds].wds_current_key);var int_key=parseInt(key);var last_pos=wds_data.length-1;if(int_curr_key>int_key){direction='left'}else if(int_curr_key==int_key){return}
...
function wds_move_dots(wds){var image_left=jQuery(".wds_slideshow_dots_active_"+wds).position().left;var image_right=jQuery(".wds_slideshow_dots_active_"+wds).position().left+jQuery(".wds_slideshow_dots_active_"+wds).outerWidth(!0);var wds_dots_width=jQuery(".wds_slideshow_dots_container_"+wds).outerWidth(!0);var wds_dots_thumbnails_width=jQuery(".wds_slideshow_dots_thumbnails_"+wds).outerWidth(!0);var long_filmstrip_cont_left=jQuery(".wds_slideshow_dots_thumbnails_"+wds).position().left;var long_filmstrip_cont_right=Math.abs(jQuery(".wds_slideshow_dots_thumbnails_"+wds).position().left)+wds_dots_width;if(!wds_params[wds].carousel){if(wds_dots_width>wds_dots_thumbnails_width){return}}
...
if(wds_params[wds].left_or_top=='top'){if(jQuery(".wds_slideshow_filmstrip_thumbnails_"+wds).position().wds_params[wds].top<0){jQuery(".wds_slideshow_filmstrip_right_"+wds).css({opacity:1,filter:"Alpha(opacity=100)"});if(jQuery(".wds_slideshow_filmstrip_thumbnails_"+wds).position().wds_params[wds].top>-wds_params[wds].filmstrip_thumb_margin_hor-wds_params[wds].filmstrip_width){jQuery(".wds_slideshow_filmstrip_thumbnails_"+wds).animate({top:0},100,'linear')}else{jQuery(".wds_slideshow_filmstrip_thumbnails_"+wds).animate({top:(jQuery(".wds_slideshow_filmstrip_thumbnails_"+wds).position().top+wds_params[wds].filmstrip_thumb_margin_hor+wds_params[wds].filmstrip_width)},100,'linear')}}
...
if(wds_params[wds].preload_images){function wds_preload(wds,preload_key){if(wds_params[wds].wds_data[preload_key].is_video=='image'){jQuery('<img />').on('load',function(){jQuery(this).remove();if(preload_key<wds_params[wds].wds_data.length-1)wds_preload(wds,preload_key+1)}).on('error',function(){jQuery(this).remove();if(preload_key<wds_params[wds].wds_data.length-1)wds_preload(wds,preload_key+1)}).attr("src",wds_params[wds].wds_data[preload_key].image_url)}else{if(preload_key<wds_params[wds].wds_data.length-1)wds_preload(wds,preload_key+1)}}
wds_preload(wds,0)}
